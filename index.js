...
function copy(arrayParam){
  let arrayCopy = arrayParam.slice();
  return arrayCopy;
}    
...
function addFlavor(arrayParam, newFlavorStr){
  arrayParam.unshift(newFlavorStr);
  return arrayParam;
}
...
function removeLastFlavor(arrayParam){
  let trashBin = arrayParam.pop();
  return arrayParam;
}
...
function getFlavorByIndex(arrayParam, indexPos){
  return arrayParam[indexPos];
}
...
function removeFlavorByName(arrayParam, flavorStr){
  let strPos = arrayParam.indexOf(flavorStr);
  arrayParam.splice(strPos, 1);
  return arrayParam;
}
...
function filterByWord(arrayParam, flavorFilter){
  let filteredFlavor = [];
  for (let i = 0; i < arrayParam.length; i++){
    if (arrayParam[i].includes(flavorFilter)){
      filteredFlavor.push(arrayParam[i]);
    }
  }
